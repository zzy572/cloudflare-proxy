...
// 用于存储合并后的代理域名列表
let proxyDomains = [...defaultProxyDomains];
...
const worker_default = {
  /**
   * @param {any} request
   * @param {{proxyip: string, proxydomains: string, pswd: string, cdnip: string, ip1: string, ip2: string, ip3: string, ip4: string, ip5: string, ip6: string, ip7: string, ip8: string, ip9: string, ip10: string, ip11: string, ip12: string, ip13: string, pt1: string, pt2: string, pt3: string, pt4: string, pt5: string, pt6: string, pt7: string, pt8: string, pt9: string, pt10: string, pt11: string, pt12: string, pt13: string}} env
   * @param {any} ctx
   * @returns {Promise<Response>}
   */
  async fetch(request, env, ctx) {
    try {
      const { proxyip, proxydomains } = env;
      // 处理proxydomains环境变量
      if (proxydomains) {
        try {
          // 将环境变量中的域名字符串转换为数组并合并到proxyDomains
          const extraDomains = proxydomains.split(',').map(domain => domain.trim());
          proxyDomains = [...new Set([...defaultProxyDomains, ...extraDomains])];
          console.log('Proxy domains:', proxyDomains);
        } catch (error) {
          console.error('Error parsing proxydomains:', error);
        }
      }
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, log) {
  // 检查是否是IP地址或者在代理域名列表中
  const isIPAddress = /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?).){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/.test(addressRemote);
  
  // 检查域名是否匹配代理列表中的任何域名（支持子域名）
  const isDomainMatch = proxyDomains.some(proxyDomain => {
    // 移除可能的尾部斜杠
    const cleanProxyDomain = proxyDomain.replace(/\/$/, '');
    // 构建匹配模式：域名结尾或者是子域名
    return addressRemote.endsWith(cleanProxyDomain) || 
           addressRemote.endsWith('.' + cleanProxyDomain);
  });
  
  const shouldUseProxy = isIPAddress || isDomainMatch;

  if (isIPAddress) {
    addressRemote = `${atob('d3d3Lg==')}${addressRemote}${atob('LnNzbGlwLmlv')}`;
  }
...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <h1>Cloudflare-workers/pages-\u0074\u0072\u006F\u006A\u0061\u006E代理脚本 V25.7.7</h1>
			<hr>
            <p>${noteshow}</p>
            <hr>
			<hr>
