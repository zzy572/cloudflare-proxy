  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (!upgradeHeader || upgradeHeader !== "websocket") {
        ...
        switch (url.pathname) {
          case `/${Pswd}`: {
            const ygkkkConfig = getygkkkConfig(
              Pswd,
...
async function handleTCPOutBound(
  ...
  if (
    ...
    addressRemote = `${atob("d3d3Lg==")}${addressRemote}${atob(
      "LnNzbGlwLmlv"
    )}`;
...
function getygkkkConfig(Pswd, hostName) {
  const wtrojanws = atob(
    btoa(
      `\u0074\u0072\u006F\u006A\u0061\u006E://${Pswd}@${CDNIP}:8880?security=none&type=ws&host=${hostName}&path=%2F%3Fed%3D2560#${hostName}`
    )
  );
  const ptrojanwstls = atob(
    btoa(
